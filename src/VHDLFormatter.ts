...
export function format(document: vscode.TextDocument, range: vscode.Range): vscode.TextEdit[] {
    const removeComments: boolean = false;
    const removeReport: boolean = false;
    const checkAlias: boolean = false;
    const keywordCase: string = 'LowerCase';
    const typeNameCase: string = 'LowerCase';
    const indentation: string = ' '.repeat(2);
    const newLineSettings: NewLineSettings = new NewLineSettings();
    newLineSettings.push("generic", "NoNewLine");
    newLineSettings.push("generic map", "NoNewLine");
    newLineSettings.push("port", "NoNewLine");
    newLineSettings.push("port map", "NoNewLine");
    newLineSettings.push(";", "NewLine");
    newLineSettings.push("then", "NewLine");
    newLineSettings.push("else", "None");
    const endOfLine: string = '\n';
    const addNewLine: boolean = true;
    ...
    const alignAllSign = true;
    const signAlignMode = 'Local';
    const isRegional = true;
    const keyWords: string[] = [];
    const alignComments = true;
    const alignSettings = new signAlignSettings(signAlignKeywords.length > 0, alignAllSign, signAlignMode, signAlignKeywords)

    let beautifierSettings = new BeautifierSettings(
        removeComments, removeReport, checkAlias, alignSettings,
        keywordCase, typeNameCase, indentation, newLineSettings, endOfLine, addNewLine
    );
    try {
        const response = beautify(document.getText(), beautifierSettings);
        return [new vscode.TextEdit(range, response)];
    }
    catch (e: unknown) {
        console.error(e);
        return [];
